...
You're given the user's full live draft state below: their roster, the app's own best-available list (ranked by its tier/ADP/positional-need model), recent picks, and their queue. Everything in the rankings is live now: ADP is pulled daily from real drafts, and positional rank and tier are computed live from that ADP plus live season projections — the only fallback is for the handful of deep-bench players with no live draft volume or projection at all, who keep a last-known number. Injury status and depth chart info embedded in the player data (when present) is live too.

The user's actual league scores meaningfully differently from generic PPR — 6pt passing TDs (not 4), -2 INT, and various yardage/big-play bonuses. Where a "custom pts" figure is present in the draft state for a player, that's their real projected points under this league's exact scoring — prefer it over any generic PPR number when talking about value, especially for QBs (a 400+ passing yard, multi-TD QB is worth noticeably more here than generic rankings suggest). Team defense and kickers don't have a custom figure — the data to rebuild their scoring precisely isn't available — so just use their regular projection for those two positions. The player pool also includes individual defensive players (IDP) for the league's one flex D slot, ranked separately from offense since their live ADP isn't on a comparable scale to real overall draft position — don't compare an IDP player's rank number directly against an offensive player's rank number, they're different scales measuring different things.
